...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pages_length > 0){
        ...
        get('numPages').innerHTML = '/ ' + pages_length;
        if (pages_length === 1){
            disableBtn('next');
        }
    }
    ...
    get('zoomOut').addEventListener('click', function() {
        pdf2htmlEX.defaultViewer.needToAdjustScaleSelect = true;
    ...
    get('scaleSelect').addEventListener('change', function() {
        var value = this.value;
        if (value === 'auto') {
            pdf2htmlEX.defaultViewer.fit_width(false, true);
        } else if (value === 'page-actual') {
            pdf2htmlEX.defaultViewer.rescale(1.0, false);
        } else if (value === 'page-fit') {
            pdf2htmlEX.defaultViewer.fit_height(true);
        } else if (value === 'page-width') {
            pdf2htmlEX.defaultViewer.fit_width(true);
        } else {
            pdf2htmlEX.defaultViewer.rescale(Number(this.value), false);
        }
    }, false);
    ...
    if (!(document.documentElement.requestFullscreen || document.documentElement.msRequestFullscreen || document.documentElement.mozRequestFullScreen || document.documentElement.webkitRequestFullscreen)) {
        addClass(presentationMode, 'hide');
    }
